...
package cn.zhuatech.attendance.config;
import cn.zhuatech.attendance.domain.DomainCatalog;
import cn.zhuatech.attendance.model.*;
import cn.zhuatech.attendance.repository.*;
...
@Configuration public class DataInitializer {
 @Bean CommandLineRunner seed(BusinessRecordRepository records,SystemSettingRepository settings,DomainCatalog catalog) {
  return args -> { if(records.count()>0)return;
            ...
            settings.save(new SystemSetting("recordNoRule", "ATTENDANCE-{YYYY}-{SEQ}"));
   ...
   int sequence=1; for(var module:catalog.modules()) {
    String no="ATTENDANCE-DEMO-"+String.format("%03d",sequence);
